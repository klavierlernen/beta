...
var blobsEnabled, metronomeInterval, lastTickTime, metronomeTolerance;
// ───────────────────────────────────────────────────
...
// Funktion, die unter dem Notensystem ein 4×4-Grid aller Notennamen anzeigt
...
// Wandle sie um in deutsche Notenschreibweise (z.B. "c#" → "Cis", "b" → "H", "eb" → "Es")
let expected;
...
choices.forEach(name => {
  ...
  btn.addEventListener("click", () => {
    if (name === expected) {
      ...
      // Markiere zusätzlich den korrekten Button (falls er noch sichtbar ist)
...
// (This ensures, sowohl bei falscher als auch bei richtiger Antwort, nach 1 Sekunde das Overlay verschwindet und die nächste Note geladen wird.)
//
// Example patch:
// elem.addEventListener('click', () => {
//   // ...existing logic...
...
// Only allow dragging if der Move-Notation-Button aktiv ist
const toggle = document.getElementById('moveNotationToggle');
...
// --- Geometriemodus-Logik für Emoji-Anzeige ---
...
setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    // Activate geometry mode to allow notation dragging
    const geometryToggle = document.getElementById('geometryToggle');
    if (geometryToggle) {
      geometryToggle.classList.add('active');
      geometryToggle.dataset.active = 'true';
    }
    const notationEl = document.getElementById('notation');
    if (notationEl) {
      notationEl.style.display = 'block';
      notationEl.style.pointerEvents = 'auto';
      notationEl.style.cursor = 'grab';
    }
...
// Verfügbare Quintenzirkel-Tonarten
const rangeArray = [
...
// Zuordnung von Tonart (Quintenzirkel) zu den ersten fünf diatonischen Noten dieser Tonart
const rangeNotes = {
"C":  ["c",  "d",  "e",  "f",  "g"],
"G":  ["g", "a", "b", "c", "d"],
"D":  ["d",  "e",  "f#", "g",  "a"],
...
if (raw.endsWith("b")) return raw.slice(0, -1) + "♭";
return raw;
}
const cycleRange = () => {
...
// (Optional) Remove or comment out the separate ➕ button outside the list, since "+" is now part of the list.
// const addPlayerToggle = document.createElement("span");
// addPlayerToggle.id = "addPlayerToggle";
// addPlayerToggle.textContent = "➕";
    ...
    <table style="width:100%; border-collapse:collapse; font-size:1.1em;">
      <thead>
        <tr>
          ...
          <th style="border-bottom:1px solid #000; padding:8px;">Ø-Score (%)</th>
        </tr>
      </thead>
      <tbody>
        ${
        ...
        <tr style="cursor:pointer; background-color:#eef;" onclick="createNewPlayerFromList()" onmouseover="this.style.background='#ddeeff'" onmouseout="this.style.background='#eef'">
          <td colspan="2" style="text-align:center; padding:12px; font-weight:bold;">➕ Neuen Spieler anlegen</td>
        </tr>
      </tbody>
    </table>
    <button onclick="closePlayerList()" style="margin-top:15px; padding:8px 16px; border:2px solid #000; border-radius:6px; background:#fff; cursor:pointer;">Zurück</button>
  </div>
`;
...
// Gibt für einen Roh-Ton und eine Lage den vollständigen VexFlow-Key zurück
function getNoteKey(rawNote, range) {
  const pitch = rawNote.toLowerCase();
  let octave = 4; // Standard-Oktave
  // Spezialfall G-Lage: C und D eine Oktave höher
...
if (window.isMobile) {
// (a) Prüfe, ob wir im Einhand-/Treble-/Bass-Modus sind
const handClef = (selectedMode === "left") ? "bass" : "treble";

// (b) Wähle genau eine zufällige Note aus rangeNotes[currentRange]
...
// G-Lage: C und D eine Oktave höher
...
// Nur für echte Quintenzirkel-Tonarten ein Key Signature setzen
...
staveNotes.forEach((note, index) => {
  ...
  text.addEventListener("click", function (event) {
    // 1. Position und Größe speichern
    ...
    // 3. Geometrie-Modus beenden / schließen
  ...
  if (noteDisplay) {
    ...
    if (kidsMode) {
      ...
      if (wizardMode) {
        ...
        const correctRaw = currentSeriesSingle[seriesCounter].note;
        // Deutsche Darstellung für Wizard-Auswahl
        let correctLetter;
        if (correctRaw.endsWith("#")) {
...
// Immer rechtsbündig und vertikal mittig platzieren
...
if (window.webkit?.messageHandlers?.customPrompt) {
  console.log("📱 iOS WebView erkannt – MIDI wird nativ behandelt.");
  return;
}
...
navigator.requestMIDIAccess({ sysex: false }).then((midiAccess) => {
  ...
  midiAccess.onstatechange = (event) => {
    ...
    if (port.type === "input" && port.state === "connected") {
      console.log("🎹 Neues MIDI-Gerät verbunden:", port.name);
...
function showNext() {
  if (phase >= steps.length) {
    ...
    setTimeout(() => {
      ...
      // Activate move-notation toggle to allow dragging
      const moveNotationToggle = document.getElementById('moveNotationToggle');
      if (moveNotationToggle) {
        moveNotationToggle.classList.add('active');
        moveNotationToggle.dataset.active = 'true';
      }
      const notationEl = document.getElementById('notation');
      if (notationEl) {
        notationEl.style.display = 'block';
...
document.addEventListener("DOMContentLoaded", () => {
  document.body.addEventListener("dblclick", () => {
    ...
    // Falls keine Funktionen existieren, kannst du stattdessen deine eigene Logik hier einfügen.
  });
});

// Separate Arrays für alte Lagen und Quintenzirkel-Tonarten
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Klick auf den Hintergrund im Mobilmodus: neue Note generieren und Korrekturanzeige öffnen
    if (window.isMobile) {
      document.body.addEventListener("click", (e) => {
        ...
        seriesCounter = 0;
        drawSeries();
        // Korrekturanzeige anzeigen über bestehende UI
        const correctRaw = currentSeriesSingle[seriesCounter].note;
        const choices = ["C","Cis","D","Dis","E","Eis","F","Fis","G","Gis","A","Ais","H"];
        // Setze expectedNote und availableNotes für vorhandene Korrekturbox
...
function updateLageProgress(correct) {
  ...
  if (correct) {
    val += 4; // z.B. 4% für richtige Note
  } else {
    val -= 8; // z.B. 8% Abzug für Fehler
  }
...
// News-Popup-Block
document.addEventListener("DOMContentLoaded", () => {
  const newsId = "v1.4-update"; // Diese ID bei jeder neuen Nachricht ändern
...
<div style="background:#fff; padding:20px; border:2px solid #000; border-radius:10px; text-align:center; max-width:400px; width:90%;">
  ...
  <table style="width:100%; border-collapse:collapse; font-size:1.1em;">
    <thead>
      <tr>
        <th style="border-bottom:1px solid #000; padding:8px;">Name</th>
        <th style="border-bottom:1px solid #000; padding:8px;">Ø (%)</th>
...
<input id="newPlayerInput" type="text" placeholder="Name eingeben…" style="margin-top:8px; padding:4px; font-size:1em;" onkeydown="if(event.key==='Enter'){submitNewPlayer();}" />
  </table>
  <button id="playerOverlayCloseBtn" style="margin-top:15px; padding:8px 16px; border:2px solid #000; border-radius:6px; background:#fff; cursor:pointer;">Zurück</button>
</div>
`;
...
// Handler für neuen Spieler anlegen (Eingabe-UI)
...
function selectPlayer(name) {
  localStorage.setItem("userName", name);
  closePlayerList();
  location.reload(); // neu laden, um Werte und Begrüßung zu aktualisieren
}
